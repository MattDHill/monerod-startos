...
This repo packages [Monero](https://github.com/monero-project/monero) for StartOS, using the third-party `sethforprivacy` images.
...
Monero ships as two coordinated images pinned by `dockerTag` in `startos/manifest/index.ts`. Both share the same Monero release and must be bumped together. See `CLAUDE.md` for the third-party-image note.

1. Bump both tags in `startos/manifest/index.ts` to the new version:
   - `ghcr.io/sethforprivacy/simple-monerod:v<new version>`
   - `ghcr.io/sethforprivacy/simple-monero-wallet-rpc:v<new version>`
